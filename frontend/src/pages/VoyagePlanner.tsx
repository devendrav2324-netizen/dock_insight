  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.response && err.response.data && err.response.data.detail) {
        setError(err.response.data.detail.error_message || JSON.stringify(err.response.data.detail));
      } else {
        setError("An unexpected error occurred while communicating with the intelligence engine.");
      }
    } finally {
      setLoading(false);
    }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', animation: 'fadeIn 0.5s ease' }}>
          ...
          <div className="grid-3">
            <div className="card">
              ...
              <div className="stat-value accent">{result.recommended_vessel.class}</div>
              <div style={{ marginTop: '0.5rem', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
                Score: <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{result.recommended_vessel.score}/100</span>
              </div>
              <p style={{ marginTop: '1rem', fontSize: '0.875rem' }}>{result.recommended_vessel.reasons[0]}</p>
            </div>
            ...
            <div className="card">
              ...
              <div className="stat-value">{result.contract_strategy.recommended_strategy}</div>
              <p style={{ marginTop: '1rem', fontSize: '0.875rem' }}>{result.contract_strategy.reasoning[0]}</p>
            </div>
            ...
            <div className="card">
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
                <div className={`stat-value ${result.risk_analysis.overall_level === 'HIGH' ? 'danger' : 'warning'}`}>
                  {result.risk_analysis.overall_score}/100
                </div>
                <div className="badge badge-mod">{result.risk_analysis.overall_level}</div>
              </div>
              <p style={{ marginTop: '1rem', fontSize: '0.875rem' }}>Dominant factor: {result.risk_analysis.dominant_risk}</p>
            </div>
          </div>
              ...
              <div style={{ display: 'flex', height: '300px' }}>
                <div style={{ flex: 1 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={[
                          { name: 'Freight', value: result.voyage_economics.breakdown.freight_cost },
                          { name: 'Bunker', value: result.voyage_economics.breakdown.bunker_cost },
                          { name: 'Port Charges', value: result.voyage_economics.breakdown.port_charges },
                          { name: 'Demurrage', value: result.voyage_economics.breakdown.demurrage_risk },
                        ]}
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  <div className="stat-box" style={{ marginBottom: '1rem' }}>
                    ...
                    <div className="stat-value">${result.voyage_economics.total_cost.toLocaleString()}</div>
                  </div>
                  <div className="stat-box">
                    <div className="stat-label">Cost per Tonne</div>
                    <div className="stat-value">${result.voyage_economics.cost_per_tonne.toFixed(2)}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          {result.explanation && (
            <div className="card" style={{ borderLeft: '4px solid var(--accent-primary)', backgroundColor: 'rgba(49, 130, 206, 0.02)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                {result.explanation.alternatives_rejected.length > 0 && (
            ...
            <div className="card">
              ...
              <div style={{ height: '300px' }}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={[
                    { day: 'T-3', rate: 14.2 }, { day: 'T-2', rate: 14.5 }, { day: 'T-1', rate: 14.8 },
                    { day: 'T0', rate: result.market_forecast.current_rate },
                    { day: 'T+1', rate: 15.3 }, { day: 'T+2', rate: 15.6 }, { day: 'T+3', rate: result.market_forecast.predicted_rate }
                  ]}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--border-color)" vertical={false} />
                    <XAxis dataKey="day" stroke="var(--text-muted)" fontSize={12} tickLine={false} axisLine={false} />
                    <YAxis stroke="var(--text-muted)" fontSize={12} tickLine={false} axisLine={false} domain={['auto', 'auto']} tickFormatter={(value) => `$${value}`} />
